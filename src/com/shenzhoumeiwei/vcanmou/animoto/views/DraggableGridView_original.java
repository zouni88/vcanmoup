...
public class DraggableGridView_original extends ViewGroup implements View.OnTouchListener, View.OnClickListener, View.OnLongClickListener {
	//layout vars
	public static float childRatio = .9f;
    protected int colCount, childSize, padding, dpi, mScroll = 0;
    ...
    public DraggableGridView_original (Context context, AttributeSet attrs) {
        super(context, attrs);
        setListeners();
        ...
        public void run()
        {
            if (dragged != -1)
            {
            	if (lastY < padding * 3 && mScroll > 0)
            		mScroll -= 20;
            	else if (lastY > getBottom() - getTop() - (padding * 3) && mScroll < getMaxScroll())
            		mScroll += 20;
            }
...
//            		+ "|R:" + getRight() + "|B:" + getBottom());
    ...
    {
        int col = getColOrRowFromCoor(x), row = getColOrRowFromCoor(y + mScroll); 
