...
/*
 * 定制自己的HttpClient,确保线程安全，提供全局访问接口
 * 自定义timeout时间等参数
 */
public class CustomHttpClient {
    ...
    public static HttpClient getNewInstance(Context mContext) {
        return getNewInstance(mContext, null);
    }

    // 每次都返回新的HttpClient实例
    public static HttpClient getNewInstance(Context mContext, CredentialsProvider cred) {
        ...
        if (null != cred) {
            ((DefaultHttpClient) newInstance).setCredentialsProvider(cred);
        }
    ...
    public static HttpResponse execute(Context context, HttpUriRequest paramHttpUriRequest)
        ...
        return execute(context, paramHttpUriRequest, null);
    }

    public static HttpResponse execute(Context context, HttpUriRequest paramHttpUriRequest,
            CredentialsProvider cred) throws ClientProtocolException, IOException {
        HttpResponse response = getNewInstance(context, cred).execute(paramHttpUriRequest);
        return response;
    }
        ...
        try {
            ...
            if (networkInfo == null || !networkInfo.isAvailable()) {
                ...
                if (netType == ConnectivityManager.TYPE_WIFI) {
                    ...
                    Cursor c = null;
                    try {
                        c = context.getContentResolver().query(PREFERRED_APN_URI, null, null, null,
                                null);
                        if (c != null) {
                            c.moveToFirst();
